...
export default function NewPostPage() {
  ...
  const handleImageFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error(err);
      alert('Failed to upload image. Make sure Cloudinary credentials are set up.');
    } finally {
  ...
  const handleSaveSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim() || !content.trim() || !selectedCategory || !imageUrl) {
      setErrorMsg('Please enter all required fields: Title, Featured Image, Category, and Content.');
      return;
    }
    ...
    <div className="space-y-8">
      ...
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link
            href="/admin/posts"
            className="p-2 rounded-xl border border-slate-200 dark:border-slate-850 hover:bg-slate-100 dark:hover:bg-slate-850 transition-colors"
          >
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight">Create Post</h1>
            <p className="text-sm text-slate-500 mt-1">Compose a new article in the rich editor</p>
          </div>
        </div>
